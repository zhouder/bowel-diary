...
开发版、预览版和体验版不是公开版本，通常只有开发者、项目成员或体验成员能打开。好友打不开时，不是代码问题，而是版本权限问题。

- 临时给少数好友用：在小程序后台把对方微信号加入“体验成员”，然后上传体验版或发体验二维码。
- 给任何微信用户使用：在微信公众平台提交审核，审核通过后发布线上版本。
- 正式发布前，需要把 `token-plan-cn.xiaomimimo.com` 加入 request 合法域名。
